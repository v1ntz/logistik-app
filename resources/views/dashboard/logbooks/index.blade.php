...
<div id="exportModal" class="hidden fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-xl w-full">
            <form id="exportForm" action="{{ route('logbooks.export') }}" method="GET">
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        <div class="mt-3 text-center sm:mt-0 sm:text-left w-full">
                            <h3 class="text-lg leading-6 font-bold text-gray-900 border-b pb-2 mb-4" id="modal-title">Export / Cetak Rekap</h3>
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500 mt-1">Gunakan ini jika ingin memisahkan rekap jenis sapi secara spesifik.</p>
                                </div>
                ...
                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse border-t gap-2">
                    <button type="submit" onclick="submitExport('excel')" class="w-full inline-flex justify-center rounded-lg border border-transparent shadow-sm px-4 py-2 bg-green-600 text-base font-bold text-white hover:bg-green-700 focus:outline-none sm:ml-3 sm:w-auto sm:text-sm">
                        📥 Download Excel
                    </button>
                    <button type="submit" onclick="submitExport('pdf')" class="w-full inline-flex justify-center rounded-lg border border-transparent shadow-sm px-4 py-2 bg-blue-600 text-base font-bold text-white hover:bg-blue-700 focus:outline-none sm:ml-3 sm:w-auto sm:text-sm">
                        🖨️ Cetak PDF Rekap
                    </button>
...
function submitExport(type) {
    ...
    if (type === 'excel') {
        form.action = "{{ route('logbooks.export') }}";
        form.target = "_self";
    } else {
        form.action = "{{ route('logbooks.rekapPdf') }}";
        form.target = "_blank";
    }
    setTimeout(() => {
