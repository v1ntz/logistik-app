@extends('layouts.app')
@section('title', 'Master Data Supplier (Luar Negeri)')
@section('content')
<div class="mb-6 flex justify-between items-center">
    <div>
        <h1 class="text-2xl font-bold text-gray-900">Master Data Supplier (Luar Negeri)</h1>
        <p class="text-gray-600 text-sm mt-1">Kelola daftar pihak eksportir/supplier asing asal muat sapi.</p>
    </div>
</div>
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6">
    <div class="md:col-span-1">
        <div class="bg-white rounded-xl shadow border border-gray-100 p-6">
            <h2 class="text-lg font-bold text-gray-800 mb-4 border-b pb-2">Tambah Supplier Baru</h2>
            <form action="{{ route('exporters.store') }}" method="POST">
                @csrf
                <div class="mb-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2">Nama Supplier (Luar Negeri)</label>
                ...
                <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded shadow transition">Simpan Supplier</button>
            </form>
        </div>
    </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Negara Asal (Lokasi)</th>
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-left">
                            <form action="{{ route('exporters.destroy', $exporter) }}" method="POST" onsubmit="return confirm('Hapus data supplier luar negeri ini?');" class="inline">
                                @csrf @method('DELETE')
                    ...
                    <tr><td colspan="3" class="px-6 py-6 text-center text-sm text-gray-500">Belum ada data master supplier luar negeri.</td></tr>
                    @endif
                </tbody>
            </table>
        </div>
